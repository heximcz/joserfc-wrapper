...
"""
The basic repository for signature keys is HashiCorp Vault, but there is also the option to store keys in standard disk files. If neither option is suitable, it is possible to write your own object for manipulating signature keys (e.g., for storing them in a database). However, this custom class must be a descendant of the abstract class AbstractKeyStorage to implement the required methods.
"""
